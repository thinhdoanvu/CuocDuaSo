...
int DetectLane::slideThickness = 10;//10
int DetectLane::BIRDVIEW_WIDTH = 240;//240
int DetectLane::BIRDVIEW_HEIGHT = 250;//320
int DetectLane::VERTICAL = 0;
int DetectLane::HORIZONTAL = 1;
Point DetectLane::null = Point();

DetectLane::DetectLane() {
    cvCreateTrackbar("LowH", "Threshold", &minThreshold[0], 179); //179
    cvCreateTrackbar("HighH", "Threshold", &maxThreshold[0], 179);//179

    cvCreateTrackbar("LowS", "Threshold", &minThreshold[1], 255);//255
    cvCreateTrackbar("HighS", "Threshold", &maxThreshold[1], 255);//255

    cvCreateTrackbar("LowV", "Threshold", &minThreshold[2], 255);//255
    cvCreateTrackbar("HighV", "Threshold", &maxThreshold[2], 255);//255

    cvCreateTrackbar("Shadow Param", "Threshold", &shadowParam, 255);//255
}
...
void DetectLane::update(const Mat &src)
{
    ...
    for (int i = 1; i < leftLane.size(); i++)
    {
        if (leftLane[i] != null)
        {
            circle(lane, leftLane[i], 1, Scalar(0,0,255), 2, 8, 0 );//280
        }
    }
    ...
    {
        if (rightLane[i] != null) {
            circle(lane, rightLane[i], 1, Scalar(255,0,0), 2, 8, 0 );//280
        }
    }
